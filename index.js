...
io.on( 'connection', function( socket )
{
  ...
  {
    people[socket.id] = {
      "name": socket.handshake.session.userName, "room": null};
    socket.emit( "update",
      "Yoooo, " + socket.handshake.session.userName + ". Please create or join a room." );
  ...
  socket.on( "joinRoom", function( name )
  {
    ...
    if( user && rooms[name] )
    {
      ...
      user.room.people_in++;
      socket.join( name );
      io.to( user.room.name ).emit( "update",
        user.name + " has joined: " + user.room.name );
    }
    else
    {
  ...
  socket.on( 'disconnect', function()
  {
    ...
    if( user )
    {
      ...
      if( user.room )
      {
        io.to( user.room.name ).emit( 'chat message',
  ...
  socket.on( 'login', function( data )
  {
    ...
    db.all( "SELECT * FROM users WHERE username='" + clean_user + "'",
      function( err, rows )
      {
        if( rows.length > 0 )
        {
          ...
          password( data.password ).verifyAgainst( row.password, function(
            error, verified )
          {
            if( verified )
            {
            ...
            {
              console.log(
                getTime() + "Password verification failed for user " + clean_user );
              socket.emit( "loginBad" );
            }
          } );
        }
  ...
  socket.on( 'create', function( data )
  {
    ...
    db.all( 'SELECT 1 FROM users WHERE username="' + clean_user + '"',
      function( err, rows )
      {
        if( rows.length > 0 )
        {
          console.log(
            getTime() + "User " + clean_user + " creation attempted again." );
